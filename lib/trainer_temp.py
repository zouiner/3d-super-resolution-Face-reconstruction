...
class Trainer(object):
    def __init__(self, config=None, rank=None, world_size=None):
        ...
        if self.cfg.enable_wandb: # No do it yet
            import wandb
    ...
    def configure_optimizers(self):
        ...
        if self.cfg.sr['model']['finetune_norm']:
            optim_params = []
            for k, v in self.model.module.sr_model.named_parameters():
            ...
            optim_params = list(self.model.module.sr_model.parameters())
    
        self.opt_sr = torch.optim.Adam(optim_params, lr=self.cfg.sr['train']["optimizer"]["lr"])
    # mica
    
        params = self.model.module.parameters_to_optimize() # !! can add into the model
    ...
    def load_checkpoint(self):
        ...
        if load_path and os.path.exists(load_path):
            ...
            if isinstance(self.model.module.sr_model, torch.nn.parallel.DistributedDataParallel):
                sr_state_dict = {f"module.{k}" if not k.startswith("module.") else k: v for k, v in sr_state_dict.items()}
            unexpected_keys = sr_state_dict.keys() - self.model.module.sr_model.state_dict().keys()
            self.model.module.sr_model.load_state_dict(sr_state_dict, strict=False)

            mica_state_dict = checkpoint['mica_model_state']
            if isinstance(self.model.module.mica_model, torch.nn.parallel.DistributedDataParallel):
                mica_state_dict = {f"module.{k}" if not k.startswith("module.") else k: v for k, v in mica_state_dict.items()}
            self.model.module.mica_model.load_state_dict(mica_state_dict, strict=False)
            ...
            if os.path.exists(gen_path):
                ...
                sr_network = self.model.module.sr_model
    ...
    def training_step(self):
        ...
        if self.cfg.sr.pretrained_model_path and  self.rank == 0:
        ...
        if self.cfg['phase'] == 'train':
            ...
            while self.global_step < n_iter + self.cfg.mica.train.max_steps: # !!take a look!! if train only SR -> set mica steps to be 0
                ...
                for _, train_data in tqdm(enumerate(self.train_iter), total=len(self.train_iter), desc="Processing training data"):
                    ...
                    l_sr, l_mica, losses, opdict = self.model(train_data, self.current_epoch, self.global_step, self.rank)
                    ...
                    if self.rank == 0:
                        ...
                        if visualizeTraining:
                            ...
                            for n in np.random.choice(range(L), size=S, replace=False):
                                ...
                                rendering = self.model.module.render.render_mesh(opdict['pred_canonical_shape_vertices'][n:n + 1, ...])
                                pred_canonical_shape_vertices = torch.cat([pred_canonical_shape_vertices, rendering])
                                rendering = self.model.module.render.render_mesh(opdict['flame_verts_shape'][n:n + 1, ...])
                                flame_verts_shape = torch.cat([flame_verts_shape, rendering])
                                input_images = torch.cat([input_images, opdict['images'].cuda()[n:n + 1, ...]])
                                if 'deca' in opdict:
                                    deca = self.model.module.render.render_mesh(opdict['deca'][n:n + 1, ...])
                        ...
                        if self.global_step % self.cfg.train.print_freq == 0:
                            logs = self.model.module.get_current_log()
            ...
            avg_ssim = 0.0
            idx = 0
            faces = self.model.module.mica_model.generator.faces_tensor.cpu()
            self.model.module.testing = True
    ...
    def prepare_data(self):
        ...
        if self.cfg.phase != 'val':
            ...
            self.sampler = DistributedSampler(self.train_dataset, num_replicas=self.world_size, rank=self.rank, shuffle=True)
