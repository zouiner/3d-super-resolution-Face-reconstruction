...
class Trainer(object):
    def __init__(self, model, config=None, device='cuda'):
        ...
        self.saveobj = self.cfg.train.save_obj
        ...
        if self.train_detail:     
            ...
            self.id_loss = lossfunc.VGGFace2Loss(pretrained_model=self.cfg.model.fr_model_path)      # cfg.model.fr_model_path = os.path.join(cfg.deca_dir, 'data', 'resnet50_ft_weight.pkl')
    ...
    def training_step(self, batch, batch_nb, training_type='coarse'):
        ...
        ###--------------- training detail model
            ...
            # add by patipol
            uv_shading = self.deca.render.add_SHlight(uv_detail_normals, codedict['light'])
            # add by patipol
    ...
    def validation_step(self):
        ...
        # images = batch['image'].to(self.device); images = images.view(-1, images.shape[-1], images.shape[-3], images.shape[-2]) 
        with torch.no_grad():
            codedict = self.deca.encode(images)
            opdict, visdict = self.deca.decode(codedict, step_global = self.global_step, step_procress = 'val')
        name = f'{self.global_step:08}'
        output_path = os.path.join(self.cfg.output_dir, self.cfg.train.val_vis_dir, name)
        os.makedirs(output_path, exist_ok=True)
        savepath = os.path.join(output_path, name + '.jpg')
        ...
        if self.saveobj:
            ...
            output_path = os.path.join(self.cfg.output_dir, self.cfg.train.val_vis_dir, name)
            os.makedirs(output_path, exist_ok=True)
            for i in range(self.batch_size*self.K):
                savepath = os.path.join(output_path, str(i+1) + '.obj')
                self.deca.save_obj(savepath, opdict, i)
        
            for key, value in opdict.items():
                if key not in ['verts', 'trans_verts', 'landmarks2d', 'landmarks3d', 'landmarks3d_world', 'grid', 'normals', ]:
                    self.grid_save(value, output_path, key)
                    for i in range(len(value)):
                        savepath = os.path.join(output_path, key)
                        temp =  (value[i].cpu().numpy().transpose(1, 2, 0)* 255).astype(np.uint8)
                        transforms.ToPILImage()(temp).save(savepath + '_' + str(i) + '.png')
    ...
    def seperate_dict(self, dict_a, index):
        dict_new = {}
        for key, value in dict_a.items():
            dict_new[key] = value[index].unsqueeze(0)
        
        return dict_new
    
    def grid_save(self, tensor_temp, output_path, variable_name, img_size = 256):
        # Initialize variables
        grid_size = int(np.ceil(np.sqrt(len(tensor_temp))))  # Calculate grid size
        grid_width = img_size * grid_size  # Assuming each image has a width of img_size pixels
        grid_height = img_size * grid_size  # Assuming each image has a height of img_size pixels
        grid_image = Image.new('RGB', (grid_width, grid_height), color=(255, 255, 255))  # Create a blank grid image
        draw = ImageDraw.Draw(grid_image)
        x_offset = 0
        y_offset = 0

        try:
            tensor_temp = tensor_temp.cpu()
        except AttributeError:
        # If tensor_temp is not a PyTorch tensor, it cannot be moved to CPU
            pass

        # Iterate through each image and paste it onto the grid
        for i, value in enumerate(tensor_temp):
            if variable_name == 'uv_z':
                value = (value - torch.min(value))/(torch.max(value)-torch.min(value))
            temp = (value.detach().numpy().transpose(1, 2, 0) * 255).astype(np.uint8)  # Convert PyTorch tensor to numpy array
            temp_image = transforms.ToPILImage()(temp)  # Convert numpy array to PIL Image
            grid_image.paste(temp_image, (x_offset, y_offset))  # Paste image onto the grid
            draw.rectangle([x_offset, y_offset, x_offset + img_size, y_offset + img_size], outline=(255, 255, 255))  # Draw boundary around each image (white line)
            x_offset += img_size  # Move to the next column
            if x_offset >= grid_width:  # Move to the next row if reached the end of the row
                x_offset = 0
                y_offset += img_size

        # Save the grid image
        savepath = os.path.join(output_path, 'grid_' + variable_name + '.png')
        grid_image.save(savepath)
    ...
    def prepare_data(self):
        ...
        print("Training data number: ", len(self.train_dataset))
        self.val_dataset = build_datasets.build_val(self.cfg.dataset)
        print("Valuating data number: ", len(self.val_dataset))
        ...
        for epoch in range(start_epoch, self.cfg.train.max_epochs):
            ...
            for step in tqdm(range(iters_every_epoch), desc=f"Epoch[{epoch+1}/{self.cfg.train.max_epochs}]"):
                ...
                if self.global_step % self.cfg.train.vis_steps == 0:
                    ...
                    name = f'{self.global_step:08}'
                    output_path = os.path.join(self.cfg.output_dir, self.cfg.train.vis_dir, name)
                    os.makedirs(output_path, exist_ok=True)
                    savepath = os.path.join(output_path, name + '.jpg')
                    grid_image = util.visualize_grid(visdict, savepath, return_gird=True)
                    # self.visual_batch(batch, output_path, name)
                    # import ipdb; ipdb.set_trace()      
                    ...
                    #     os.makedirs(output_path, exist_ok=True)
                    #     for i in range(self.batch_size*self.K):
                    #         opdict_new = self.seperate_dict(opdict, i)
                    #         savepath = os.path.join(output_path, str(i+1) + '.obj')
                    #         self.deca.save_obj(savepath, opdict_new)
                    
                    # It cannot use in train because of opdict isn't as same as in val
    ...
    def visual_batch(self, batch, savepath, name):
        ...
        # Assuming B is the batch size and K is the number of images per batch
        B, K, _, _ = images.shape

        for i in range(B):
            image = images[i].cpu().numpy().transpose((1, 2, 0))  # Convert from PyTorch format to NumPy format
            landmark = lmk[i].cpu().numpy()
            mask = masks[i].cpu().numpy()

            # Save the image with landmarks using Matplotlib
            plt.figure()
            plt.imshow(image)
            plt.scatter(landmark[:, 0], landmark[:, 1], s=10, c='r', marker='o')  # Assuming (x, y) landmarks
            plt.title(f"GT Image {i + 1} with Landmarks")
            plt.axis('off')

            # Save the image with landmarks
            output_path = os.path.join(savepath, name + '_' + f"GTimage_with_landmarks_{i + 1}.png")
            plt.savefig(output_path)
            plt.close()

            # Save the mask
            mask_output_path = os.path.join(savepath, name + '_' + f"GTmask_{i + 1}.png")
            plt.imsave(mask_output_path, mask, cmap='gray')
    ...
    def save_img(self, img, save_path):
        ...
        for i in range(img.shape[0]):
            ...
            # Transpose the color channel dimension (assuming channels are in the first dimension)
            single_image = np.transpose(single_image.cpu().numpy(), (1, 2, 0))

            # Rescale pixel values to the range [0, 255] (assuming 0-1 float values)
            single_image = (single_image * 255).astype(np.uint8)

            # Save the image using imsave
            plt.imsave(save_path + f'_{i + 1}.png', single_image)