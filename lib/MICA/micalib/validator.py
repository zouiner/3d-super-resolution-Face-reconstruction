...
from lib.MICA.utils.best_model import BestModel
...
class Validator(object):
    ...
    def run(self):
        with torch.no_grad():
            ...
            self.trainer.tb_logger.add_scalar('val/average', average, global_step=self.trainer.global_step)
            for key in avg_per_dataset.keys():
                l, i = avg_per_dataset[key]
                avg = l.item() / i
                self.trainer.tb_logger.add_scalar(f'val/average_{key}', avg, global_step=self.trainer.global_step)

            # Save best model
            smoothed_weighted, smoothed = self.best_model(weighted_average, average)
            self.trainer.tb_logger.add_scalar(f'val/smoothed_average', smoothed, global_step=self.trainer.global_step)

            # self.now()

            # Print embeddings every nth validation step
            if self.trainer.global_step % (self.cfg.mica.train.val_steps * 5) == 0:
            ...
            if self.trainer.global_step % self.cfg.mica.train.val_save_img != 0:
                return
            ...
            savepath = os.path.join(self.cfg.output_dir, self.cfg.mica.train.val_vis_dir, f'{self.trainer.global_step:08}.jpg')
    ...
    def now(self):
        ...
        self.trainer.tb_logger.add_scalar(f'val/now_mean', mean, global_step=self.trainer.global_step)
