...
class mockDataset_old(Dataset):
    def __init__(self, K, image_size, scale, trans_scale = 0, isEval = False, isTest = False):
        ...
        # self.workspace_path = '/shared/storage/cs/staffstore/ps1510/Tutorial/Dataset/CelebA_mock_split/train'
        # if isEval:
        ...
        if os.path.exists(os.path.join(self.workspace_path, 'labels/dataline_4.npy')):
            self.data_lines = np.load(os.path.join(self.workspace_path, 'labels/dataline_4.npy')).astype('str')
        else:
    ...
    def __getitem__(self, idx):
        ...
        for i in random_ind:
            ...
            if name[len(self.imagefolder)+1:] != '':
