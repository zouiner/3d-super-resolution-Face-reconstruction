...
def is_img_file(file_path): # check this file is an image
    _, extension = os.path.splitext(file_path)
    return any(extension.lower() == i for i in IMG_EXTENSIONS)
...
def prepare_process_worker(wctx, file_subset, order = False):
...
def prepare(img_path, out_path, n_worker, sizes=(16, 128), resample=Image.BICUBIC, lmdb_save=False, order = False):
    ...
    if not os.path.exists(img_path + '_pathlist.npy'):
        # Find all files and filter out only image files
        files = [p for p in Path(img_path).rglob('*') if is_image_file(p)]
        files.sort()
        # Save the list of image file paths
        np.save(img_path + '_pathlist.npy', files)
    else:
        # Load the list of image file paths
        files = np.load(img_path + '_pathlist.npy', allow_pickle=True).tolist()
        files.sort()
        ...
        for file in tqdm(files):
            ...
            if imgs is not None:
                ...
                if not lmdb_save:
                    if order:
                        lr_img.save(
                            '{}/lr_{}/{}_{}.png'.format(out_path, sizes[0], str(total+1).zfill(fill), i.zfill(5)))
                        hr_img.save(
                            '{}/hr_{}/{}_{}.png'.format(out_path, sizes[1], str(total+1).zfill(fill), i.zfill(5)))
                        sr_img.save(
                            '{}/sr_{}_{}/{}_{}.png'.format(out_path, sizes[0], sizes[1], str(total+1).zfill(fill), i.zfill(5)))
                    else:
                        lr_img.save(
...
if __name__ == '__main__':
    ...
    parser.add_argument('--order', '-od', action='store_true')
    ...
    prepare(args.path, args.out, args.n_worker,
            sizes=sizes, resample=resample, lmdb_save=args.lmdb, order = args.order)
...
# python data/prepare_data.py  --path /shared/storage/cs/staffstore/ps1510/Tutorial/Dataset/VGG-Face2/DECA_setting/eval_all  --out /shared/storage/cs/staffstore/ps1510/Tutorial/Image-Super-Resolution-via-Iterative-Refinement/contents/vgg_face2_eval --size 8,16 -od
